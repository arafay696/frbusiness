...
if ( !class_exists('Revo_Widget') ):

abstract class Revo_Widget extends WP_Widget{
	...
	protected function init(){
		...
		$this->base_tpl_path = apply_filters('revo_widget_template_base', $this->base_path.'/widgets/'.$this->id_base.'/tmpl');
		$this->override_tpl_path = apply_filters('revo_widget_override_base', get_template_directory().'/widgets/'.$this->id_base);
	}
	...
	public function revo_trim_words( $text, $num_words = 30, $more = null ) {
	...
	public function widget_template_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( array_key_exists('ov', $templates) && count($templates['ov']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Override by Theme', 'revo') .'">';
			foreach ($templates['ov'] as $name => $label) {
				$select_html .= '<option value="' . esc_attr( $name ) . '"';
		...
		if ( array_key_exists('df', $templates) && count($templates['df']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Default Template', 'revo') .'">';
	...
	public function category_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( (is_string($opts['multiple']) && strtolower($opts['multiple'])=='multiple') || (is_bool($opts['multiple']) && $opts['multiple']) ){
			...
			if (array_key_exists('allow_select_all', $opts) && $opts['allow_select_all']){
				...
				$allow_select_all = '<option value="0">'. esc_html_e( 'All Categories', 'revo' ) .'</option>';
			}
		}
	...
	public function form( $instance ){
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title', 'revo')?></label>
			<br />
			<input class="widefat"
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('widget_template') ); ?>"><?php esc_html_e("Template", 'revo') ?></label>
			<br/>
