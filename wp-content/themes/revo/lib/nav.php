...
class Revo_Menu_Admin {
	...
	static function options_setup() {
		self::$custom_fields = array(
			'mega_active' => array(
					'type' => 'checkbox',
					'label' => esc_html__( 'Active mega menu', 'revo' ),
					'default' => 0					
			),
			'icon' => array(
					'type' => 'text',
					'label' => 'Icon',
					'description' => sprintf( __('Show an icon by <a href="%s">Font Awesome 4.6</a>','revo'), esc_url( 'http://fortawesome.github.com/Font-Awesome/' ) ),
					'default' => '',
					'preview' => true,
			),
			'span' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Dropdown sub size', 'revo' ),
					'default' => '',
					'options' => array(
						'' => 'default',
						'col-1' => '1',
			...
			'mega_full' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Active Full Mega Menu', 'revo' ),
					'default' => 0					
			),
			'dropdown_span' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Dropdown Size', 'revo' ),
					'default' => '',
					'options' => array(
						'column-1' => '1',
						'column-2' => '2',
			...
			'show_description_as_subtitle' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Show Description as Subtitle', 'revo' ),
					'default' => 0					
			),
			'hide_title' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Hide Title', 'revo' ),
					'description' =>  esc_html__( 'If icon or subtitle are using.', 'revo' ),
					'default' => 0
			),
			'disable_link' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Disable Link', 'revo' ),
					'description' =>  esc_html__( 'Remove Link', 'revo' ),
					'default' => 0
			),
			'advanced' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Advanced', 'revo' ),
					'default' => '',
					'options' => array(
							'no' =>  esc_html__( 'Default', 'revo' ),							
							'apc' =>  esc_html__( 'Append Advanced Content or Shortcode', 'revo' ),
							'apcs' =>  esc_html__( 'Append Advanced Content with Page Select', 'revo' )
					)
			),
			'advanced_content' => array(
					'type' => 'textarea',
					'label' =>  esc_html__( 'Advanced Content', 'revo' ),
					'default' => '',
					'depends' => 'advanced_eq_rbd'
			),
			'page_select' => array(
					'type' => 'page_select',
					'label' =>  esc_html__( 'Page Select', 'revo' ),
					'default' => 0,
					'description' =>  esc_html__( 'Select page to show on mega menu', 'revo' ),					
			),
			'imgupload'		   => array(
					'type'	=> 'upload',
					'label' =>  esc_html__( 'Menu Thumbnail', 'revo' ),
					'default' => ''
			),
		);
	}
}
add_action( 'admin_init', array('Revo_Menu_Admin','options_setup' ) );
if ( !class_exists('Revo_Menu') ):
...
class Revo_Menu {
	public function __construct(){
		...
		add_action( 'admin_enqueue_scripts', array( $this,'revo_menu_upload') );
	}
	function revo_menu_upload(){
		$current_screen =  get_current_screen();	
		if( isset( $current_screen->id ) && $current_screen->id == 'nav-menus' ){
			wp_enqueue_script(
				'revo-menu-upload-js',get_template_directory_uri().'/lib/admin/js/field_upload.js',
				array('jquery', 'thickbox', 'media-upload'),
				time(),
				true
			);	
			wp_enqueue_style('thickbox'); // thanks to https://github.com/rzepak
			wp_localize_script('revo-menu-upload-js', 'revo_upload', array('url' => REVO_URL.'/admin/img/placeholder.png') );
		}
	}

	public function add_custom_nav_fields( $menu_item ){
		if ( count(Revo_Menu_Admin::$custom_fields) ){
			foreach (Revo_Menu_Admin::$custom_fields as $field => $field_opts){
	...
	public function update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ){
		if ( count(Revo_Menu_Admin::$custom_fields) ){
			foreach (Revo_Menu_Admin::$custom_fields as $field => $type){
	...
	public function edit_walker($walker, $menu_id){
		return 'Revo_Menu_Admin_Walker';
	}
}

endif;

if ( !class_exists('Revo_Menu_Admin_Walker') ):
 ...
 */
class Revo_Menu_Admin_Walker extends Walker_Nav_Menu  {
	/**
	...
	function start_el(&$output, $item, $depth = 0, $args = Array(), $current_object_id = 0) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'revo' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__( '%s (Pending)','revo' ), $item->title );
		}
				...
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'revo'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'revo'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e('Edit Menu Item', 'revo'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php esc_html_e( 'Edit Menu Item', 'revo' ); ?></a>
					</span>
				</dt>
			</dl>
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Navigation Label','revo' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Title Attribute','revo' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
						...
						<?php esc_html_e( 'Open link in a new window/tab','revo' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'CSS Classes (optional)','revo' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)','revo' ); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Description','revo' ); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr( $item_id ); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','revo'); ?></span>
					</label>
				</p>
				<div class="advance-menu">
					<a href="javascript:void(0);" class="menu-advance-href" data-id="menu-advance-<?php  echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Show/Hide Advance Menu Config', 'revo'); ?> </a>
								...
								switch( $field_opts['type'] ):
									...
									case 'select':
										...
										<?php if ( isset($field_opts['options']) && is_array($field_opts['options']) ): ?>
											...
											><?php echo ( sprintf( _n( '%s', '%s', $opt_label, 'revo' ), $opt_label ) ); ?></option>
									...
									case 'page_select':
									?><select  id="edit-menu-item-<?php echo esc_attr( $field ); ?>-<?php echo esc_attr( $item_id ); ?>" class="edit-menu-item-<?php echo esc_attr( $field ); ?>" name="menu-item-<?php echo esc_attr( $field ); ?>[<?php echo esc_attr( $item_id ); ?>]" >
										<option value="0" selected><?php esc_html_e( 'Select page', 'revo' ); ?></option>
										<?php $pages = get_pages(); ?>
									...
									?>			
											...
											<?php
												echo sprintf( ( $item->$field != '') ? '<img class="revo-opts-screenshot" id="revo-opts-screenshot-'.esc_attr( $item_id ).'" src="%s" width = "30" />': '', esc_url( $item->$field ) );
											if($item -> imgupload == ''){$remove = ' style="display:none;"';$upload = '';}else{$remove = '';$upload = ' style="display:none;"';}
											echo ' <a href="javascript:void(0);" class="revo-menu-upload button-secondary"'.$upload.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.esc_html__('Browse', 'revo').'</a>';
											echo ' <a href="javascript:void(0);" class="revo-menu-upload-remove"'.$remove.' rel-id="edit-menu-item-'.esc_attr( $field ).'-'.esc_attr( $item_id ).'">'.esc_html__('Remove', 'revo').'</a>';
											?>								
									<?php
										break;	
										case 'checkbox':
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __('Original: %s','revo'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php esc_html_e('Remove','revo'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php	echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
						?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e('Cancel','revo'); ?></a>
				</div>
...
if ( !class_exists('Revo_Menu_Walker') ):

class Revo_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if ( isset($item->icon) && !empty($item->icon) ){
			...
			if( $depth == 0 ){
				$item_html = preg_replace('/(<a[^>]*>)(.*)(<\/a>)/iU', '$1<span class="menu-title">$2</span><span class="menu-img"><img src="'.esc_url($item->imgupload).'" alt="'. esc_attr__( 'Thumbnail', 'revo' ) .'"/></span>$3', $item_html);
			}
		}
		else {
...
if ( !class_exists('Revo_Mega_Menu_Walker') ):

class Revo_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if( ($depth === 0) ){
			$class_names .= '';
			$class_names .= ( $item -> mega_active == 1 ) ? ' revo-mega-menu' : ' revo-menu-custom';
			$class_names .= ( $item -> mega_full == 1 ) ? ' revo-megamenu-full' : '';
			$class_names .= ' level1';
		}		
		if( $item -> imgupload != '' ){
			$class_names .= ' revo-menu-img';
		}
		if( $item -> icon != '' ){
			$class_names .= ' revo-menu-icon';
		}
		...
		if ( $show_icon || $show_title || $show_subtitle || $show_img ){
			...
			if( $show_img ){
				$span_img = '<span class="menu-img"><img src="'. esc_url( $item -> imgupload ) .'" alt="'. esc_attr__( 'Menu Image', 'revo' ) .'" /></span>';
			}else{
				$span_img = '';
			}
			if ( $show_subtitle ){
...
add_filter('nav_menu_css_class', 'revo_nav_menu_css_class', 10, 2);
 ...
 * Use Revo_Menu_Walker() by default
*/
...
add_filter('wp_nav_menu_args', 'revo_nav_menu_args');
